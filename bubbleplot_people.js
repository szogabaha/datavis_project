...
function mapPerson(e, acc, obj, position) {
  {
...
// Split the list by position, crop top N objects in each list
// and merge the result (result: a merged list of
/// top N director, top N editor, top N musician)
...
function getBubblePeopleData(data) {
  let unsorted = data.reduce((acc, obj) => {
    ...
    directors.forEach(e => mapPerson(e, acc, obj, "director"));
    const editors = getPeopleFromObject(obj["Edited by"]);
    editors.forEach(e => mapPerson(e, acc, obj, "editor"));
    const musicians = getPeopleFromObject(obj["Music by"]);
    musicians.forEach(e => mapPerson(e, acc, obj, "musician"));
...
function updateBubblePeople(data) {
  ...
  const root = BUBBLE_CONFIG.pack(d3.hierarchy({ children: transformedData })
    .sum(d => d.revenue));
  const bubbles = BUBBLE_CONFIG.bubbles.data(root.leaves());
...
function plotBubblePeople(data, width = 600, height = 500) {
  ...
  const node = svg.append("g")
    ...
    .attr("transform", d => `translate(${d.x},${d.y})`)
    .on('click', d => {
      if(BUBBLE_CONFIG.selectedPeople.filter(x => x.name == d.data.name && x.position == d.data.position).length === 0) {
        BUBBLE_CONFIG.selectedPeople.push(d.data);
      } else {
        BUBBLE_CONFIG.selectedPeople = BUBBLE_CONFIG.selectedPeople.filter(x => !(x.name == d.data.name && x.position == d.data.position));
      }
